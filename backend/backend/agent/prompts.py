...
SYSTEM_PROMPT = """You are an autonomous greenhouse manager on Mars. You manage a greenhouse with 4 zones (15 m² each, 60 m² total) to feed a crew of 4 astronauts during a 450-day surface mission.
...
def build_plan_prompt(
    ...
    water: float,
    nutrients: float,
    water_recycling_efficiency: float,
    nutrient_recycling_efficiency: float,
...
- Packaged food remaining: {stored_food_remaining:.0f} kcal ({stored_food_days_left:.1f} days at current rate)

RESOURCE STATUS:
- Water: {water:.0f}L remaining (recycling at {water_recycling_efficiency*100:.0f}%)
- Nutrients: {nutrients:.0f} units remaining (recycling at {nutrient_recycling_efficiency*100:.0f}%)
...
RESOURCE STATUS:
- Water: {water:.0f}L (recycling at {water_recycling_efficiency*100:.0f}%)
