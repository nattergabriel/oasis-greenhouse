...
from src.models import Environment, Zone


def _make_zones(num_lit: int = 4) -> list[Zone]:
    """Helper: create zones with a given number of lit ones."""
    return [
        Zone(id=i, artificial_light=(i <= num_lit))
...
class TestSolarHours:
    ...
    def test_day_0_near_baseline(self) -> None:
        env = Environment()
        update_environment(env, day=0, zones=_make_zones())
    ...
    def test_summer_peak(self) -> None:
        ...
        update_environment(env, day=peak_day, zones=_make_zones())
    ...
    def test_winter_low(self) -> None:
        ...
        update_environment(env, day=low_day, zones=_make_zones())
    ...
    def test_solar_hours_range_over_year(self) -> None:
        ...
        for day in range(config.MARTIAN_YEAR_SOLS):
            update_environment(env, day=day, zones=_make_zones())
...
class TestEnergyBudget:
    ...
    def test_no_deficit_at_baseline(self) -> None:
        """Day 0 with 4 lit zones should have manageable energy budget."""
        env = Environment()
        update_environment(env, day=0, zones=_make_zones())
    ...
    def test_deficit_in_winter(self) -> None:
        ...
        low_day = 3 * config.MARTIAN_YEAR_SOLS // 4
        update_environment(env, day=low_day, zones=_make_zones())
        # solar ~9h → generated ~31.5
        # heating: 0.5 * (22 - (-83)) ≈ 52.5
        # lighting: 4 * 2.0 = 8
        # pumps: 1.0
        # needed ≈ 61.5 → deficit ≈ 30
    ...
    def test_no_deficit_in_summer(self) -> None:
        ...
        peak_day = config.MARTIAN_YEAR_SOLS // 4
        update_environment(env, day=peak_day, zones=_make_zones())
        # solar ~15h → generated ~52.5
        # heating: 0.5 * (22 - (-43)) ≈ 32.5
        # lighting: 8, pumps: 1 → needed ≈ 41.5
        # Should have surplus
        assert env.energy_deficit == pytest.approx(0.0, abs=1.0)

    def test_fewer_lit_zones_reduces_energy_cost(self) -> None:
        env_4lit = Environment()
        env_2lit = Environment()
        update_environment(env_4lit, day=0, zones=_make_zones(num_lit=4))
        update_environment(env_2lit, day=0, zones=_make_zones(num_lit=2))
        assert env_2lit.energy_needed < env_4lit.energy_needed
        diff = env_4lit.energy_needed - env_2lit.energy_needed
        assert diff == pytest.approx(2 * config.LIGHTING_COST_PER_ZONE, abs=0.01)

    def test_heating_cost_zero_if_colder_inside(self) -> None:
        """Edge case: if internal temp < outside temp, no heating needed."""
        # Both target and internal must be set (apply_events resets internal=target)
        env = Environment(target_temp=-90.0, internal_temp=-90.0)
        update_environment(env, day=0, zones=_make_zones(num_lit=0))
