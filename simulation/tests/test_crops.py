...
from src.models import Crop, Environment, FoodItem, ResourcePool, Slot
...
def _optimal_slot() -> Slot:
    return Slot(id=1, artificial_light=True, water_allocation=1.0)
...
class TestGrowthEfficiency:
    ...
    def test_optimal_conditions_return_one(self) -> None:
        ...
        eff = calculate_growth_efficiency(crop, env, _full_resources(), _optimal_slot())
    ...
    def test_no_water_returns_zero(self) -> None:
        ...
        eff = calculate_growth_efficiency(crop, env, resources, _optimal_slot())
        assert eff == 0.0
    ...
    def test_no_artificial_light_halves_solar(self) -> None:
        ...
        slot_lit = Slot(id=1, artificial_light=True, water_allocation=1.0)
        slot_dark = Slot(id=1, artificial_light=False, water_allocation=1.0)
        eff_lit = calculate_growth_efficiency(crop, env, _full_resources(), slot_lit)
        eff_dark = calculate_growth_efficiency(crop, env, _full_resources(), slot_dark)
    ...
    def test_heat_stress_reduces_efficiency(self) -> None:
        ...
        eff_ok = calculate_growth_efficiency(crop, env_ok, _full_resources(), _optimal_slot())
        eff_hot = calculate_growth_efficiency(crop, env_hot, _full_resources(), _optimal_slot())
        assert eff_hot < eff_ok

    def test_unknown_crop_type_returns_zero(self) -> None:
        crop = Crop(id="x", type="alien", slot_id=1, footprint_m2=1.0, planted_day=0)
        eff = calculate_growth_efficiency(crop, Environment(), _full_resources(), _optimal_slot())
        assert eff == 0.0
...
class TestGrowth:
    ...
    def test_potato_reaches_harvest_in_cycle(self) -> None:
        ...
        env = _optimal_env_for("potato")
        for _ in range(90):
            grow_crop(crop, env, _full_resources(), _optimal_slot())
    ...
    def test_zero_water_no_growth(self) -> None:
        ...
        env = _optimal_env_for("potato")
        resources = ResourcePool(water=0.0, nutrients=5_000.0, water_availability=0.0)
        grow_crop(crop, env, resources, _optimal_slot())
    ...
    def test_half_efficiency_roughly_half_growth(self) -> None:
        ...
        for _ in range(20):
            grow_crop(crop_full, env_full, _full_resources(), _optimal_slot())
            grow_crop(crop_half, env_half, _full_resources(), _optimal_slot())
    ...
    def test_lettuce_faster_than_potato(self) -> None:
        ...
        env = Environment(solar_hours=12.0, internal_temp=18.0, light_penalty=0.0)
        for _ in range(30):
            grow_crop(lettuce, env, _full_resources(), _optimal_slot())
            grow_crop(potato, env, _full_resources(), _optimal_slot())
...
class TestStressDetection:
    ...
    def test_no_stress_at_optimal(self) -> None:
        crop = _make_crop("potato")
        env = _optimal_env_for("potato")
        stress = detect_stress(crop, env, _full_resources(), _optimal_slot())
        assert stress is None

    def test_drought_when_water_factor_low(self) -> None:
        crop = _make_crop("potato")
        env = _optimal_env_for("potato")
        slot = Slot(id=1, water_allocation=0.3)  # below 0.5 threshold
        stress = detect_stress(crop, env, _full_resources(), slot)
        assert stress == "drought"

    def test_overwatering_when_water_factor_high(self) -> None:
        crop = _make_crop("potato")
        env = _optimal_env_for("potato")
        slot = Slot(id=1, water_allocation=1.4)  # above 1.3 threshold
        stress = detect_stress(crop, env, _full_resources(), slot)
    ...
    def test_nutrient_deficiency_when_low(self) -> None:
        ...
        stress = detect_stress(crop, env, resources, _optimal_slot())
    ...
    def test_light_insufficient_when_dark(self) -> None:
        ...
        slot = Slot(id=1, artificial_light=False, water_allocation=1.0)
    ...
    def test_drought_takes_priority_over_heat(self) -> None:
        ...
        slot = Slot(id=1, water_allocation=0.3)  # also drought
        stress = detect_stress(crop, env, _full_resources(), slot)
...
class TestHarvest:
    ...
    def test_unknown_crop_yields_nothing(self) -> None:
        crop = Crop(id="x", type="alien", slot_id=1, footprint_m2=1.0,
