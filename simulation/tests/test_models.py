...
class TestSerializationRoundtrip:
    ...
    def test_empty_state_roundtrip(self, default_state: GreenhouseState) -> None:
        ...
        assert len(restored.zones) == len(default_state.zones)
    ...
    def test_populated_state_roundtrip(self, populated_state: GreenhouseState) -> None:
        ...
        assert len(restored.zones[0].crops) == 1
        assert restored.zones[0].crops[0].id == "crop_1_1"
        assert restored.zones[0].crops[0].type == "potato"
        assert restored.zones[0].crops[0].age == 45
        assert restored.zones[0].crops[0].health == 87.0
        assert restored.zones[0].crops[0].growth == 50.0
        assert restored.zones[0].crops[0].active_stress is None
        assert restored.zones[0].crop_plan == {"potato": 0.6, "beans_peas": 0.4}

        assert len(restored.zones[1].crops) == 1
        assert restored.zones[1].crops[0].active_stress == "heat"
        assert restored.zones[1].artificial_light is False
        assert restored.zones[1].water_allocation == 0.8
...
class TestZoneMethods:
    """Zone area calculations."""

    def test_empty_zone_full_area_available(self) -> None:
        zone = Zone(id=1, area_m2=15.0, crops=[])
        assert zone.used_area() == 0.0
        assert zone.available_area() == 15.0

    def test_used_area_sums_footprints(self) -> None:
        potato = Crop(id="c1", type="potato", zone_id=1, footprint_m2=2.0,
                      planted_day=0, growth_cycle_days=90)
        lettuce = Crop(id="c2", type="lettuce", zone_id=1, footprint_m2=0.5,
                       planted_day=0, growth_cycle_days=37)
        zone = Zone(id=1, area_m2=15.0, crops=[potato, lettuce])
        assert zone.used_area() == 2.5
        assert zone.available_area() == 12.5

    def test_can_plant_checks_available_area(self) -> None:
        zone = Zone(id=1, area_m2=2.0, crops=[])
        assert zone.can_plant("potato") is True    # 2.0 m² fits in 2.0
        assert zone.can_plant("lettuce") is True   # 0.5 fits
        # Fill it with a potato
        zone.crops.append(Crop(id="c1", type="potato", zone_id=1,
                               footprint_m2=2.0, planted_day=0, growth_cycle_days=90))
        assert zone.can_plant("potato") is False   # 0.0 left
        assert zone.can_plant("herbs") is False    # 0.3 > 0.0

    def test_can_plant_unknown_crop_returns_false(self) -> None:
        zone = Zone(id=1, area_m2=15.0, crops=[])
        assert zone.can_plant("alien_fruit") is False
